...
Note: if I can not obtain the original stimuli in time I will substitute the stimuli with two different sets that differ in their visual complexity. 
...
- The current format of breaks between blocks are prespecified intervals, and the breaks between tasks are self-paced. This is subject to change until the authors respond with more specifics. 
...
- Data exclusion rules: In Daffner et al.(2000) the exclusion rule is based on criteria related to the EEG measurement, which is unobtainable with the online format. The current replication experiment will exclude a trial if the RT is more than 3 standard deviation away, and we will exclude the participant if a) one presses the space bar for more than non-target 25% trials or b) one does not press the space bar for more than 25% of the target trial or c) indicate one has a history of neurological disorder or learning disabilities or d) missing data from more than 25% trials after applying the trial-based exclusion rule. 
...
See [00_read_raw_data](https://github.com/psych251/daffner2000/blob/master/writeup/daffner2000_analysis/script/00_read_raw_data.R)
...
####Import data <- current no exclusion rule applied 
processed_data <- read_csv(here("writeup/daffner2000_analysis/processed_data/processed_data.csv"))
...
data_for_analysis <- processed_data %>% 
...
Confirmatory analysis: 
```{r}
anova_test(data = data_for_analysis, 
           formula = trial_looking_time ~ stimuli_type * task_type,
