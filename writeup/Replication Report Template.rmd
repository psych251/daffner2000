...
- Data exclusion rules: In Daffner et al.(2000) the exclusion rule is based on criteria related to the EEG measurement, which is unobtainable with the online format. The current replication experiment will exclude a trial if the RT is more than 3 standard deviation away, and we will exclude the participant if a) one presses space bar for more than 25% trials or b) indicate one has history of neurological disorder or learning disabilities or c) missing more than 25% trials. 
